...
% MAXREGIONS    = Options.max_regions; 自己改的
MAXREGIONS    = 2000
...
size(G,1)
% G
if ~isempty(bndA), %如果有和参数无关的约束
    crA = [zeros(size(bndA,1),nu) bndA; G -S];
    crb = [bndb; W];
    crA_index = [1: size(bndA,1), Matrices.G_star_index];
else
    crA = [G -S]; % constraint matrix的系数矩阵
    crb = W; 
    crA_index = Matrices.G_star_index;
end
...
G     = crA(keptRows,1:nu); %keptRows是一个集合

G_crA_index = crA_index(1,keptRows)
                ...
                if aSetFound,
                    ...
                    if ( ~sameASFound ),
                        if ( useAdjacency & niter<=ALPHAit ),
                            ...
                            fprintf('reducedMatrices')
                        else
                            cr = getCriticalRegion (probMatrices, activeSet, crOptions);
                            fprintf('probMatrices')
%                             return;
                        end
...
%-----------------------------------------------------------------------------
    ...
    G            = matrices.G;  %promatrices.G, 削减过后的cra
    S            = matrices.S;
    W            = matrices.W;
    [nConstr,nu] = size(G);
    ...
    slacks = abs(Bineq - Aineq * xoptLP); % inequality
    idxActive   = find( slacks < Options.zeroTol );
    idxCompl    = find(abs(lambda.ineqlin(idxActive)) > Options.zeroTol); % 第idxActive的dual是否等于0
...
reducedMatrices = reduxMatrices; %cra削减过后的
%
fullG  = fullMatrices.G; %整个未削减cra对应的
...
reducedMatrices.G = [reduxMatrices.G(idxActive,:); ...
    fullG(idxInactiveReduced,:)]; %削减过后的cra的有效和未削减的cra的inactive
