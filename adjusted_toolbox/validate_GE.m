...
%     end   
    ...
    Hn = [-opt.pB opt.A opt.b]; % 不等式约束 A*z <= b + pB*x, z是决策变量, x是参数
...
%     Hn = [Hn; opt.Ath zeros(size(opt.Ath,1),opt.n) opt.bth]; %把参数的约束加进去
    
    % store internally merged constraints % used in eliminateEquations and qp2lcp
    opt.Internal.pB = -Hn(:,1:opt.d); %d: 参数的维度, n: 决策变量的维度
    opt.Internal.A = Hn(:,opt.d+1:opt.n+opt.d); %保存初始值
...
%          P = Polyhedron(Hn(:, 1:end-1), Hn(:, end)); %不考虑等式约束
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 这一块是被我注释啦		
% 		if P.isEmptySet
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        fprintf('for the sample from operating range feasibility_check\n')
            return;
        end
         
        fprintf('validate_minHRep\n')
		[P, hull,extrem_points_num, removed_count, screening_time_box, screening_time_line, screening_time,num_line_box] = P.minHRep();
        opt.Internal.extrem_points_num = extrem_points_num;
        opt.Internal.removed_count = removed_count;
        opt.Internal.screening_time_box = screening_time_box;
        opt.Internal.screening_time_line = screening_time_line;
        opt.Internal.screening_time = screening_time;
        opt.Internal.num_line_box = num_line_box;
		P.outerApprox;
		lb = P.Internal.lb;
		ub = P.Internal.ub;
        ...
        if any(lb(1:opt.d)==Inf) || any(ub(1:opt.d)==-Inf),
            ...
            % equalities
            P = Polyhedron('H', Hn, 'He', [-opt.pE opt.Ae opt.be]);
            fprintf('validate_minHRep-Parameter bounds\n')
            [P, hull] = P.minHRep(); %找到redundant constraints
            P.outerApprox;
            lb = P.Internal.lb;
            ub = P.Internal.ub;
        ...
        opt.Internal.A = hull.H(:,opt.d+1:opt.n+opt.d);%除掉了redundant之后的约束
...
%%       opt.Internal.removed_rows.lower = [opt.Internal.removed_rows.lower; remained.lb(hull.I(opt.m+1:opt.m+nnz(~ilb)))]; %%我注释掉的 因为我把上面的ub lb注释掉啦
...
%             opt.bth = [];        
%         else
%             fprintf('Inequalities-parameter\n')
